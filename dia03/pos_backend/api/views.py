...
class PedidoView(APIView):
    def get(self,request):
        ...
        seriPedido = PedidoSerializerGET(dataPedido,many=True)
        
        context = {
            'ok':True,
            'pedidos':seriPedido.data
        }
        
        return Response(context)
    
    def post(self,request):
        seriPedido = PedidoSerializerPOST(data=request.data)
        seriPedido.is_valid(raise_exception=True)
        seriPedido.save()
        
        return Response({'status':True,'content':seriPedido.data})